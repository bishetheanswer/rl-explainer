...
BREAKOUT_RAW_PROMPT = """
You are an expert Atari game commentator. I have provided a video
clip of a reinforcement learning agent playing an Atari game.
...
Based only on what you see in the video, answer the question after Q:.
Mention timestamps and game elements if necessary.

Don't answer until I pass the question after Q:.
"""
BREAKOUT_RAW_VIDEO_URL = "https://youtube.com/shorts/me6EL4Y1hHQ"

# Based on the video, describe the primary strategy the agent has learned.
# Is there a specific, advanced tactic it seems to be using repeatedly?
...
Based only on what you see in the video and the saliency maps, answer the question after Q:.
Mention timestamps and game elements if necessary. Pay attention to the saliency maps,
a brighter color means the agent was paying more attention to that part of the frame.
The agent might be paying attention to multiple parts of the frame at the same time.
Do not mention the saliency maps explicitly in your response.

Don't answer until I pass the question after Q:.
"""
BREAKOUT_SALIENCY_VIDEO_URL = "https://youtube.com/shorts/mtPXLNlpqi0"
...
class GeminiBreakoutChat:
    def __init__(self, model: str) -> None:
        ...
        self._saliency_chat.send_message(
            message=[
                types.Part(
    ...
    def send_message(
        self, message: str
    ) -> tuple[GenerateContentResponse, GenerateContentResponse]:
        raw_response = self._raw_chat.send_message(f"Q: {message}")
        saliency_response = self._saliency_chat.send_message(f"Q: {message}")

        return raw_response, saliency_response

    def send_message_and_print_responses(self, message: str) -> None:
        raw_response, saliency_response = self.send_message(message)
